  ...
  implements ElfProgramHeaderInterface
{
  ...
  constructor(
    ...
    let [elfProgramHeader, last_offset] = this.parse(
      buffer,
      e_phnum,
      e_phoff,
      e_phentsize,
    ...
    // set data
    this.data = new DataView(buffer, e_phoff.raw_dec, last_offset);
  }
  ...
  private parse(
    ...
    e_phoff: ElfData,
    e_phentsize: ElfData,
  ): [ElfProgramHeaderEntry[], number] {
    let last_offset = 0;
    let phdr_entries: ElfProgramHeaderEntry[] = [];

    for (
      let phdr_entry_count = 0;
      phdr_entry_count < e_phnum.raw_dec;
      phdr_entry_count++
    ) {
